...
int main(){
	...
	std::vector <std::string> models = { "accent 1996 - 2001", "accent 2001 - 2012", "accent 2012 - 2016", "Audi A3 Sedan Attraction", "Audi A3 Sportback Ambition", "Audi A3 Sportback Attraction", "BMW 3.20", "Volkwagen Golf", "Fiat Doblo",
	...
	std::string svm_file_name = "svm_20170305_1";
	std::string vocabulary_file_name = "vocabulary_20170305_1";
	...
	std::string test_path = "C:/Users/raistlin/Documents/Visual Studio 2013/Projects/ModelDetection/App/App/Model/test";

	ofstream myfile;
	myfile.open("log.txt");
	std::vector<cv::String> filenames;
	cv::glob(test_path, filenames);

	for (int i = 0; i < filenames.size(); ++i){
		cv::Mat input_image = imread(filenames[i]);
		md->set_image(input_image);

		std::cout << "Remaining: " << filenames.size()- i +1 << std::endl;
		int prediction = md->detect_model();

		myfile << "File: " << filenames[i] << "\t" << "Label: " << models[prediction - 1]<< "\t" <<prediction << std::endl;
